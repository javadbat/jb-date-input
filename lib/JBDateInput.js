...
class JBDateInputWebComponent extends HTMLElement {
    ...
    static get observedAttributes() {
        return ['label', 'value-type', 'message', 'value', 'name', 'format', 'min', 'max'];
    }
    ...
    setMinDate(dateString) {
        if(this.valueType == "GREGORIAN"){
            ...
            this.dateRestrictions.min = dayjs(`${dateValueObj.groups.year}-${dateValueObj.groups.month}-${dateValueObj.groups.day}`)
        }
        if(this.valueType == "JALALI"){
            const dateValueObj = this.getDateObjectValueBaseOnFormat(dateString, this.format);
            this.dateRestrictions.min = dayjs(`${dateValueObj.groups.year}-${dateValueObj.groups.month}-${dateValueObj.groups.day}`,{jalali:true})
        }
        if(this.valueType == "TIME_STAMP"){
            this.dateRestrictions.min = dayjs(parseInt(dateString));
        }
    ...
    setMaxDate(dateString) {
        if(this.valueType == "GREGORIAN"){
            const dateValueObj = this.getDateObjectValueBaseOnFormat(dateString, this.format);
            this.dateRestrictions.max = dayjs(`${dateValueObj.groups.year}-${dateValueObj.groups.month}-${dateValueObj.groups.day}`)
        }
        if(this.valueType == "JALALI"){
            const dateValueObj = this.getDateObjectValueBaseOnFormat(dateString, this.format);
            this.dateRestrictions.max = dayjs(`${dateValueObj.groups.year}-${dateValueObj.groups.month}-${dateValueObj.groups.day}`,{jalali:true})
        }
        if(this.valueType == "TIME_STAMP"){
            this.dateRestrictions.max = dayjs(parseInt(dateString));
        }
